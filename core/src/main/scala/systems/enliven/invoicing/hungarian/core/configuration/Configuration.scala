 ...
 */
abstract class Configuration[Specialized <:
                             Configuration[Specialized] :
                             ClassTag :
                             Factory.forConfiguration](
...
extends Logger with Serializable {
   ...
   */
  def getAll: Map[String, String] = configuration.entrySet().asScala.toList.map(
   ...
   */
  def objectList(key: String): List[collection.Map[String, String]] = try {
   ...
   * Be aware.
   */
  def get[T: TypeTag](key: String, logger: T => Unit = (_: T) => ()): T = try {
    val value = (typeOf[T] match {
      ...
      case t if t =:= typeOf[List[String]] => configuration.getList(key).unwrapped().asScala.toList
      case t if t =:= typeOf[Array[String]] => configuration.getList(key).unwrapped().asScala.toArray
      ...
      case t if t =:= typeOf[Map[String, List[String]]] =>
        configuration.getObject(key).unwrapped().asScala.mapValues(_.asInstanceOf[java.util.ArrayList[String]].asScala.toList).toMap
    ...
    typeOf[T] match {
      case t if t =:= typeOf[List[String]] =>
        newConfiguration.setUnderlyingConfiguration(
          configuration.withValue(key, ConfigValueFactory.fromIterable(value.asInstanceOf[List[String]].asJava)))
      case _ =>
        newConfiguration.setUnderlyingConfiguration(
  ...
  private def mergeConfigurations(): Config = {
    ...
    _resource.entrySet().asScala.foreach {
      pair => {
        conf = conf.withValue(pair.getKey, pair.getValue)
      }
    }
    conf
  }
  ...
  def saveToWorking() = {
    ...
    Files.write(Paths.get(path), underlyingConfiguration
      .root()
...
object Configuration {
  protected[configuration] var nInstances = new {
    ...
    def updateInstanceCount(className: String): Int = synchronized {
