...
object Hash {
  ...
  def hashSHA512(string: String): String =
    MessageDigest.getInstance("SHA-512").digest(string.getBytes(StandardCharsets.UTF_8)).map("%02x".format(_)).mkString.toUpperCase

  def hashSHA3512(string: String): String =
    new SHA3.Digest512().digest(string.getBytes(StandardCharsets.UTF_8)).map("%02x".format(_)).mkString.toUpperCase

  implicit object InvoiceOperationListHash extends Hash[InvoiceOperationListType] {
    def hashed(payload: InvoiceOperationListType): String = payload.invoiceOperation
